...
# Display the summary statistics for the selected weather type
st.subheader(f"Summary Statistics for {weather_filter} Weather")
...
# Display the scatter plot for the selected weather type
fig, ax = plt.subplots()
sns.scatterplot(x='temperature', y='count', data=selected_data, ax=ax)
plt.title(f'Temperature vs. Bike Loans for {weather_filter} Weather')
...
# Table
st.write('Average Bike Count by Day Type')
st.dataframe(average_count_by_day_type)
